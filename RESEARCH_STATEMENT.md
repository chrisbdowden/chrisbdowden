...
Univariate AUC Ranking — Each feature scored independently against outcome
No hyperparameters to tune
Fully interpretable (direction + strength of association)
Information barrier: feature ranking sees only training labels
Adaptive Selection — Predefined thresholds (AUC ≥ 0.55, p < 0.05, max panel size)
Declared before validation
No cohort-specific modifications
Transparent decision rules
Weighted Probability Fusion — AUC-weighted z-score composition with direction correction
...
---
Open Questions
How does BPF perform on single-cell RNA-seq data? (High-dimensional, sparse)
Can fusion synthesis extend to multi-modal imaging + genomics? (Spatial omics)
What's the optimal panel size for clinical deployment? (Cost vs. accuracy tradeoff)
Can prodromal PD detection translate to prospective clinical trials? (Next frontier)
---
Let's Collaborate
